...
const EditorPage: React.FC = () => {
  const { html, css, js, activeTab, setHtml, setCss, setJs, setActiveTab } = useEditor();
    ...
    <main className="h-screen flex flex-col bg-gray-50" style={{ fontFamily: 'Mona Sans, ui-sans-serif, system-ui' }}>
      ...
      <div className="flex-1 flex min-h-0 gap-1 p-1">
        ...
        <section className="flex-1 flex flex-col min-w-0 bg-white rounded-lg shadow-sm border border-gray-200">
          <EditorTabs tabs={TABS} activeTab={activeTab} setActiveTab={setActiveTab} />
          <div className="flex-1 min-h-0">
            <CodeEditor
              language={TABS.find(t => t.key === activeTab)?.language || 'html'}
